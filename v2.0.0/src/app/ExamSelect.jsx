...
import { fetchExams, updateProfile } from "@/lib/api";
...
export default function ExamSelect() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const updated = await updateProfile(user.id, { target_exams: selected });
      updateUser({ ...user, target_exams: selected });
        ...
        {exams.map((e, i) => {
          ...
          return (
            <motion.button
              ...
              className={`text-left rounded-3xl p-6 border-2 transition-all ${on ? "border-subject-matematik bg-subject-matematik/5" : "border-zinc-200 bg-white hover:border-zinc-300"}`}
            >
