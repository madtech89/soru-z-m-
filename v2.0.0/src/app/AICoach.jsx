...
export default function AICoach() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    fetchDashboard(user.id).then(setD).catch(() => setD(false));
    fetchLatestAIRecommendation(user.id).then(setAi).catch(() => {});
  }, [user?.id]);
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: EASE }}>
        <Card className="p-7 mb-5 bg-subject-ai/5 border-subject-ai/20 relative overflow-hidden">
          ...
          <div className="relative flex items-start gap-4">
            <span className="h-12 w-12 rounded-2xl bg-subject-ai/15 grid place-items-center shrink-0"><Brain size={22} className="text-subject-ai" /></span>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-heading font-bold text-lg">Kişisel analiz</h3>
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-subject-ai/15 text-subject-ai">{ai ? "AI" : "kural tabanlı"}</span>
              </div>
