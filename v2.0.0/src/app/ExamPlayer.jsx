...
import { fetchTest, startTestSession, submitTestSession } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import { Spinner, EASE, LoginPrompt } from "@/app/ui";
...
export default function ExamPlayer() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (saved && saved.sessionId) {
        ...
        const s = await startTestSession(testId, user.id);
  ...
  }, [testId, lsKey, user?.id]);

  const persist = useCallback((next = {}) => {
    localStorage.setItem(lsKey, JSON.stringify({ sessionId, answers, marked, endAt, time: timeRef.current, ...next }));
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => { timeRef.current[qid] = (timeRef.current[qid] || 0) + 1; }, 1000);
    return () => clearInterval(iv);
  }, [idx, test]);
  ...
  const handleSubmit = async (auto = false) => {
    ...
    setSubmitting(true);
    try {
      const result = await submitTestSession(sessionId, testId, user.id, answers, timeRef.current);
      localStorage.removeItem(lsKey);
      if (auto) toast.info("Süre doldu, deneme otomatik gönderildi.");
      nav("/app/sonuc", { state: { result } });
    } catch {
  ...
  if (!user) return <LoginPrompt title="Deneme çözmek için giriş yap" message="Deneme sonucun kaydedilsin, puanın hesaplansın ve ilerlemeni takip edebilesin." />;
