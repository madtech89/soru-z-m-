...
function QuestionCard({ q, subjectSlug }) {
  ...
  const answer = async (o) => {
    if (res) return;
    setSel(o);
    const { data } = await api.post("/practice/answer", { question_id: q.id, selected_answer: o, time_spent: 0 });
    setRes(data);
  };
...
export default function QuestionBank() {
  ...
  useEffect(() => {
    ...
    api.get(`/exams/${f.exam_id}/subjects`).then((r) => {
      setSubjects(r.data);
  ...
  const load = useCallback(() => {
    ...
    Object.entries(f).forEach(([k, v]) => {
      if (!v) return;
      params.append(k === "result_filter" ? "result_filter" : k, v);
    });
          ...
          <div className="space-y-4">
            {data.items.map((q, i) => (
              <motion.div key={q.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04, ease: EASE }}>
                <QuestionCard q={q} subjectSlug={subjMap[q.subject_id]} />
              </motion.div>
            ))}
          </div>
