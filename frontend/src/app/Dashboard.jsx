...
import { tone, statusColor } from "@/lib/subjects";
import { Card, StatCard, Spinner, EASE } from "@/app/ui";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    api.get("/dashboard").then((r) => setD(r.data)).catch(() => setD(false));
  }, []);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3">
          <span className="flex items-center gap-2 bg-subject-sosyal/10 text-subject-sosyal font-semibold px-4 py-2 rounded-full text-sm">
            <Flame size={16} /> {d.streak} gün seri
          </span>
          <span className="flex items-center gap-2 bg-subject-matematik/10 text-subject-matematik font-semibold px-4 py-2 rounded-full text-sm">
            <Zap size={16} /> {d.xp} XP
          </span>
        </div>
      </div>
          ...
          <Card className="p-7 h-full relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex items-center gap-2 text-sm text-zinc-500 mb-1">
                <Target size={16} className="text-subject-matematik" /> Bugünkü Hedef
              </div>
              ...
              <div className="h-3 rounded-full bg-zinc-100 overflow-hidden">
                <motion.div
                  className="h-full rounded-full bg-subject-matematik"
              ...
              <div className="flex items-center justify-between mt-3 text-sm">
                ...
                <Link to="/app/soru-bankasi" className="text-subject-matematik font-semibold flex items-center gap-1">
                  Soru çöz <ArrowRight size={15} />
                </Link>
              </div>
            </div>
          </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mt-5">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5, ease: EASE }}>
          <Card className="p-6 h-full bg-subject-ai/5 border-subject-ai/20 flex flex-col">
            <span className="h-10 w-10 rounded-xl bg-subject-ai/15 grid place-items-center mb-4">
              <Brain size={19} className="text-subject-ai" />
            </span>
            <h3 className="font-heading font-bold text-lg">AI Koç önerisi</h3>
            <p className="text-sm text-zinc-500 mt-2 flex-1">
              {d.weak_topics[0]
                ? `${d.weak_topics[0].topic_name} konusunda başarın %${d.weak_topics[0].proficiency}. Önce ders notunu incele, ardından 20 soru çöz.`
                : "Birkaç deneme çöz, kişisel önerini oluşturalım."}
            </p>
            <Link to="/app/ai-koc" className="mt-4 text-subject-ai font-semibold text-sm flex items-center gap-1">
              AI Koç'a git <ArrowRight size={15} />
            </Link>
          </Card>
        </motion.div>
      </div>
            ...
            {d.weak_topics.map((t) => (
              <div key={t.topic_id}>
                ...
                <div className="h-2 rounded-full bg-zinc-100 overflow-hidden">
                  <motion.div className="h-full rounded-full" style={{ background: statusColor(t.status) }}
                    initial={{ width: 0 }} whileInView={{ width: `${t.proficiency}%` }} viewport={{ once: true }} transition={{ duration: 0.8, ease: EASE }} />
                </div>
              </div>
            ))}
          </div>
            ...
            {d.recommended_tests.map((t) => (
              <Link key={t.id} to={`/app/deneme/${t.id}`} className="flex items-center justify-between p-3 rounded-xl border border-zinc-200 hover:border-subject-matematik hover:bg-subject-matematik/5 transition-colors group">
                <div className="flex items-center gap-3">
                  <span className="h-9 w-9 rounded-lg bg-subject-matematik/10 grid place-items-center">
                    <FileText size={16} className="text-subject-matematik" />
                  </span>
                  <div>
                    <div className="text-sm font-semibold">{t.name}</div>
                    <div className="text-xs text-zinc-400">{t.question_count} soru · {t.duration_minutes} dk</div>
                  </div>
                </div>
