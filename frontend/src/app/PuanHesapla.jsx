...
export default function PuanHesapla() {
  ...
  useEffect(() => { api.get("/exams").then((r) => { setExams(r.data); if (r.data[0]) setExamId(r.data[0].id); }).catch(() => setExams([])); }, []);

  useEffect(() => {
    if (!examId) return;
    setResult(null);
    setCfg(null);
    api.get(`/exams/${examId}/scoring`).then((r) => {
      const c = r.data || { sections: [], base_score: 100, multiplier: 1, score_type: "Ham Puan" };
      setCfg(c);
  ...
  const calc = async () => {
    setBusy(true);
    try {
      const { data } = await api.post("/score/calculate", { exam_id: examId, sections: rows });
      setResult(data);
