...
import { BookOpen, PlayCircle, FileText, X, ExternalLink } from "lucide-react";
...
function NoteModal({ note, subjSlug, onClose }) {
  const token = localStorage.getItem("netor_token");
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}
      ...
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
        ...
        <div className="p-6">
          ...
          {note.file_path && (
            <a href={`${API}/files/${note.file_path}?auth=${token}`} target="_blank" rel="noreferrer" data-testid="note-file"
              className="inline-flex items-center gap-2 mb-5 px-4 py-2 rounded-xl font-medium text-white" style={{ background: t.hex }}>
              <FileText size={16} /> {note.file_name || "Dokümanı aç"} <ExternalLink size={14} />
            </a>
          )}
...
export default function DersNotlari() {
  ...
  useEffect(() => {
    ...
    if (examId) q.append("exam_id", examId);
    if (topicFilter) q.append("topic_id", topicFilter);
    api.get(`/notes?${q}`).then(async (r) => {
      setNotes(r.data);
      if (topicFilter && r.data[0]) setActive(r.data[0]);
    }).catch(() => setNotes([]));
  }, [examId, topicFilter]);
  ...
  useEffect(() => {
    ...
    api.get(`/exams/${examId}/subjects`).then((r) =>
      setSubjMap(Object.fromEntries(r.data.map((s) => [s.id, s.slug]))));
  }, [examId]);
