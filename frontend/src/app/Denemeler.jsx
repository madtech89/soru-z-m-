...
export default function Denemeler() {
  ...
  useEffect(() => {
    ...
    api.get(`/tests${q}`).then((r) => setTests(r.data));
  }, [examId]);
    ...
    <div>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 mb-6">
        <button onClick={() => setExamId("")} data-testid="deneme-filter-all"
          className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-colors ${examId === "" ? "bg-ink text-white border-ink" : "border-zinc-300 text-zinc-600 hover:border-ink"}`}>
          Tümü
        </button>
        {exams.map((e) => (
          <button key={e.id} onClick={() => setExamId(e.id)} data-testid={`deneme-filter-${e.name}`}
            className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-colors ${examId === e.id ? "bg-ink text-white border-ink" : "border-zinc-300 text-zinc-600 hover:border-ink"}`}>
            {e.name}
          </button>
        ))}
      </div>
      ...
      {tests === null ? (
        ...
        <Empty text="Bu sınav için henüz deneme eklenmemiş." />
      ) : (
          ...
          {tests.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04, duration: 0.4, ease: EASE }}>
              <Card className="p-6 h-full flex flex-col">
                <span className="h-11 w-11 rounded-xl bg-subject-matematik/10 grid place-items-center mb-4">
                  <FileText size={19} className="text-subject-matematik" />
                </span>
                ...
                <div className="flex items-center gap-4 text-xs text-zinc-400 mt-4 mb-4">
                  <span className="flex items-center gap-1"><HelpCircle size={13} /> {t.question_count} soru</span>
