...
- Ders Notları: study_notes modeli + admin CRUD + dosya yükleme (Yerel ve bulut dosya depolama) + video URL + konu bazlı otomatik yönlendirme (Eksiklerim/AI Koç → ilgili not). Kullanıcı ders notları sayfası + modal.
...
- AI Koç (LLM): Gerçek performans verisine dayalı Türkçe analiz + 7 günlük çalışma planı (`/api/ai/coach`), ai_recommendations kaydı (OpenAI / Akıllı Fallback desteği).
- Mobil menü çekmeceyle tüm sayfalar erişilebilir; PuanHesapla null-config koruması.
- Testing iterasyon 2: 82/82 backend pytest pass; AI Koç çağrısı doğrulandı.
